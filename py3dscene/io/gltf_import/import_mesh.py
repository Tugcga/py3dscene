...
def get_polygon_indices(model: GLTFModel, primitive: GLTFPrimitive, first_index: int) -> list[int]:
...
def add_shape_target(gltf_model: GLTFModel,
    ...
    if shape_name in gltf_shape:
        ...
        if shape_accessor.sparse.is_sparse:
            ...
            shape_values = get_float_buffer(gltf_model, shape_accessor)
...
def import_object_mesh(gltf_model: GLTFModel,
    ...
    for primitive_index in range(len(gltf_mesh.primitives)):
        ...
        positions: list[float] = get_float_buffer(gltf_model, position_accessor)
        if len(positions) == 0:
            continue

        triangles: list[int] = get_polygon_indices(gltf_model, gltf_primitive, 0)
        ...
        for attribute_name, attribute_acc in gltf_primitive.attributes.items():
            ...
            if attribute_name == "NORMAL":
                normals: list[float] = get_float_buffer(gltf_model, accessor)
                ...
                uvs: list[float] = get_float_buffer(gltf_model, accessor)
                ...
                joints: list[int] = get_integer_buffer(gltf_model, accessor)
                skin_joints.extend(joints)
            elif attribute_name.find("WEIGHTS") == 0:
                weights: list[float] = get_float_buffer(gltf_model, accessor)
                skin_weights.extend(weights)
            elif attribute_name == "TANGENT":
                tangents: list[float] = get_float_buffer(gltf_model, accessor)
        ...
        for shape_index in range(len(gltf_primitive.targets)):
            ...
            add_shape_target(gltf_model, gltf_shape, "POSITION", mesh)
