...
from py3dscene.bin.tiny_gltf import TINYGLTF_TARGET_ELEMENT_ARRAY_BUFFER
from py3dscene.bin.tiny_gltf import TINYGLTF_TARGET_ARRAY_BUFFER
...
def add_data_to_buffer(gltf_model_buffers_data: list[int],
    ...
    view: GLTFBufferView = GLTFBufferView()
    view.buffer = 0
    view.byte_offset = len(gltf_model_buffers_data)
    view.byte_length = len(byte_vector)
    if not ignore_target:
        view.target = TINYGLTF_TARGET_ELEMENT_ARRAY_BUFFER if is_indices else TINYGLTF_TARGET_ARRAY_BUFFER
    ...
    accessor: GLTFAccessor = GLTFAccessor()
...
def add_float_to_buffer(gltf_model_buffers_data: list[int],
                        ...
                        component_type: int,
                        data_type: int):
    items_count: int = len(data) // get_num_components_in_type(data_type)
    byte_vector: list[int] = []
    for v in data:
